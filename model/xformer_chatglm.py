...
def xformer_efficient_attention_fn(
    ...
    # seqlen, batch, num_attention_heads, hidden_size_per_attention_head
    q_seq_len, b, num_attention_heads, _ = query_layer.shape
...
class SelfAttention(torch.nn.Module):
    ...
    def forward(
        ...
        context_layer, present, attention_probs = self.attenion_function(
            self=self,
...
class ChatGLMForConditionalGenerationXformer(ChatGLMPreTrainedModel):
    def __init__(self, config: ChatGLMConfig, empty_init=True):
        super().__init__(config)
        if empty_init:
            init_method = skip_init
